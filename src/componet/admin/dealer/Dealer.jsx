...
const Dealer = () => {
  ...
  const deletesrv = async (deletid) => {
    try {
      ...
      await deleteDoc(doc(db, "Dealers", deletid));

      console.log("Dealers delet successfully!");
      toast.success("Dealers delet successfully!");
      showAll()
    } catch (error) {
      console.error("Error delet Dealers:", error);
      toast.error("Error delet Dealers:", error);
    }
  }
  ...
  const updatesrv = async () => {
    try {
      ...
      const productRef = doc(db, "Dealers", updtid); // ✅ Properly create a document reference
      ...
      console.log("Dealers update successfully!");
      toast.success("Dealers Update successfully!");
      showAll()

    } catch (error) {
      console.error("Error updating Dealers:", error);
      toast.error("Error updating Dealers:", error);
    }
  };
  ...
  const addoneservice = async () => {
    try {
      ...
      const productRef = doc(db, "Dealers", addsrvid); // ✅ Properly create a document reference
      ...
      toast.success("Dealers added successfully!");
      showAll()
    } catch (error) {
      console.error("Error adding Dealers:", error);
      toast.error("Error adding Dealers:", error);
    }
  }
  //###################################################################### one Service add end
  ...
  return (
    <div>
      ...
      <div className="container mt-4">
        <h2 className="mb-4">Dealer Management</h2>

        <button className="btn btn-dark mb-3" data-bs-toggle="modal" data-bs-target="#addDealerModal">
          <i className="fas fa-tools"></i> Add Dealer
        </button>
            ...
            <tbody>
              ...
              {serviceall.map((srv, index) => (
                <tr key={srv.id}>
                  ...
                  <td>{srv.location}</td>
                  <td className="action-icons">
                    <i className="fas fa-edit edit" data-bs-toggle="modal" data-bs-target="#editDealerModal" onClick={() => editsrv(srv.id)}></i>
      ...
      <div className="modal fade" id="addDealerModal" tabIndex="-1" aria-labelledby="addUserModalLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="addUserModalLabel">Add Dealer</h5>
      ...
      <div className="modal fade" id="editDealerModal" tabIndex="-1" aria-labelledby="editUserModalLabel"
        aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="editUserModalLabel">Edit Dealer</h5>
              <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">
              <form onSubmit={e => e.preventDefault()}>
